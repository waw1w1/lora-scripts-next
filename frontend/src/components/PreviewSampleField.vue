...
const props = withDefaults(defineProps<{ samples?: string[]; editing?: boolean; disabled?: boolean }>(), { samples: () => [], editing: false, disabled: false })
    ...
    <div v-for="(sample, index) in decoded.samples" :key="index" class="preview-sample-item">
      ...
      <div class="sample-settings">
        <label v-for="key in settings" :key="key">
          ...
          <el-input-number :model-value="sample[key]" :min="key === 'seed' || key === 'guidance_scale' ? 0 : 1" :step="key === 'guidance_scale' ? 0.1 : 1" :disabled="disabled" @update:model-value="update(index, { [key]: $event ?? sample[key] })" />
        </label>
      </div>
