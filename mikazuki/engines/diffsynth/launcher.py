...
def build_train_spec(runtime, config_path, gpu_ids=None):
    ...
    command = [str(runtime.python), "-m", "accelerate.commands.launch", "--num_processes", "1", "--num_machines", "1", "--mixed_precision", "bf16", "--dynamo_backend", "no", str(Path(__file__).with_name("entry.py").resolve()), "--project-root", str(runtime.project_root), "--config", str(config_path)]
    env = process_env()
