...
def handle_run(config, ctx):
    ...
    try:
        ...
        spec = build_train_spec(rt, adapted.arguments, ctx.gpu_ids)
        ui_config = Path(ctx.autosave_dir) / f"{run_id}.toml"
        ui_config.write_text(toml.dumps({**config, "model_train_type": TRAIN_TYPE}), encoding="utf-8")
        metadata = {
            ...
            "total_steps": len(adapted.dataset) * adapted.arguments["dataset_repeat"] * adapted.arguments["num_epochs"],
            "warnings": ["当前入口不支持训练中采样预览；保存的检查点为 LoRA 权重，不含优化器状态。"],
        }
